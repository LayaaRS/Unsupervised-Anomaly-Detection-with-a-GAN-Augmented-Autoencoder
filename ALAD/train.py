...
for epoch in range(num_epochs):
    ...
    #   torch.save(dis.state_dict(), filepath + 'D.pt')
        ...
        print("epoch: %d, iteration: %d, discriminator loss: %.2f, generator loss: %.2f, \
               encoder loss: %.2f" %(epoch, i, loss_discriminator.item(), loss_generator.item(),
                                     loss_encoder.item()))        

print('End of training')

# save model
filepath = str(args.save_model_dir) + str(args.dataset) + str(num_epochs) + 'epochs' + \
